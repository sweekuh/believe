...
deeper ideas hiding under the jokes.
...
```
believe/
├── index.html               # the app: loads episodes.json, renders cards, gate
...
Rules:
- **Stable `id` per card** (format `s{season}e{episode}-{slug}`). Keep ids
  unique and don't reuse them.
